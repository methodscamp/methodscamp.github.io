...
1.  Add a new column to the data frame, called `diff_agree`, which subtracts `agree` and `agree_pred`. How would you create `abs_diff_agree`, defined as the absolute value of `diff_agree`?
...
`ggplot2` is the package in charge of data visualization in the `tidyverse`. It is extremely flexible and allows us to draw bar plots, box plots, histograms, scatter plots, and many other (see [examples at R Charts](https://r-charts.com/ggplot2/)).
...
::: callout-tip
If we exchange the variables in the `x` and `y` aesthetic mappings, we can get a horizontal plot:
